...
public class GraphServer implements Runnable
{
	...
	private void sendAllInfoMessage(ClientConnection client)
	{
		...
		client.sendMessage(message);
		client.sendMessage(NetworkProtocol.SET_PREVIEW+"&"+(previewEnabled ? 1 : 0));
		client.sendMessage(NetworkProtocol.SET_TURN_TIME+"&"+turnTime);
		client.sendMessage(NetworkProtocol.SET_TRAJECTORY_MODE+"&"+trajectoryMode);
		client.sendMessage(buildMapMessage());
	}
	...
	private int clampTurnTime(int value)
	{
		if(value < Constants.MIN_TURN_TIME)
		{
			return Constants.MIN_TURN_TIME;
		}
		if(value > Constants.MAX_TURN_TIME)
		{
			return Constants.MAX_TURN_TIME;
		}
		return value;
	}

	private void sendRoomSettings()
	{
		sendMessageAll(NetworkProtocol.SET_PREVIEW+"&"+(previewEnabled ? 1 : 0));
		sendMessageAll(NetworkProtocol.SET_TURN_TIME+"&"+turnTime);
		sendMessageAll(NetworkProtocol.SET_TRAJECTORY_MODE+"&"+trajectoryMode);
		sendMessageAll(buildMapMessage());
	}

	private String buildMapMessage()
	{
		String message = NetworkProtocol.SET_MAP+"&"+(customMapEnabled ? 1 : 0)+"&"+customMap.size();
		ListIterator<MapShape> itr = customMap.listIterator();
		while(itr.hasNext())
		{
			String[] fields = itr.next().encode().split(",");
	...
	private int clampTrajectoryMode(int value)
	{
		if(value != Constants.GLOBAL_TRAJECTORY)
		{
	...
	private List<MapShape> parseMapMessage(String[] info)
	{
		try
		{
			if(info.length < 3)
			...
			int shapeCount = Integer.parseInt(info[2]);
			if(shapeCount < 0 || shapeCount > MapShape.MAX_SHAPES || info.length != 3 + 5*shapeCount)
			{
				return null;
			}
			...
			{
				int offset = 3 + 5*i;
				shapes.add(MapShape.decode(Integer.parseInt(info[offset]), Integer.parseInt(info[offset+1]),
						Integer.parseInt(info[offset+2]), Integer.parseInt(info[offset+3]), Integer.parseInt(info[offset+4])));
			}
			if(enabled == 0 && shapeCount != 0)
			{
				return null;
			}
			return shapes;
	...
	{				
		...
		for(int i=0; i<numCircles; i++)
		{
			...
			while(circles[3*i+2] < 1)
			{
				circles[3*i+2] = (int)(random.nextGaussian()*Constants.CIRCLE_STANDARD_DEVIATION+Constants.CIRCLE_MEAN_RADIUS);
			}
			circles[3*i+2] = Math.min(circles[3*i+2], Math.min(Constants.PLANE_LENGTH, Constants.PLANE_HEIGHT)/2 - 1);
			circles[3*i] = circles[3*i+2] + random.nextInt(Constants.PLANE_LENGTH - 2*circles[3*i+2]);
			circles[3*i+1] = circles[3*i+2] + random.nextInt(Constants.PLANE_HEIGHT - 2*circles[3*i+2]);
	...
	private boolean testSoldier(Soldier soldier, List<Soldier> soldiers, List<MapShape> shapes)
	{
		ListIterator<Soldier> itr = soldiers.listIterator();
		while(itr.hasNext())
		{
		...
		ListIterator<MapShape> shapeItr = shapes.listIterator();
		while(shapeItr.hasNext())
		{
			if(shapeItr.next().intersects(soldier.x, soldier.y, Constants.SOLDIER_SELECTION_RADIUS))
			{
				return false;
			}
		}
	...
	private Soldier generateSoldier(List<Soldier> soldiers, List<MapShape> shapes, int team)
	{
	...
	private int[] generateSoldiers(List<MapShape> shapes)
	{
		...
		while(pitr.hasNext())
		{
			...
			{
				Soldier soldier = generateSoldier(soldiers, shapes, player.getTeam());
				if(soldier == null)
				{
					return null;
				}
		...
		{
			soldiersPos[2*i] = soldiers.get(i).x;
			soldiersPos[2*i+1] = soldiers.get(i).y;
		}
		return soldiersPos;
	}

	private boolean canPlaceCustomMap(List<MapShape> shapes)
	{
		return generateSoldiers(shapes) != null;
	}
	...
	protected void startGame()
	{
		...
		if(customMapEnabled)
		{
			...
			if(soldiers == null)
			{
				...
				shapes = new MapShape[circles.length/3];
				for(int i=0; i<shapes.length; i++)
				{
					shapes[i] = MapShape.circle(circles[3*i], circles[3*i+1], circles[3*i+2]);
				}
				soldiers = generateSoldiers(circles);
			}
		}
		else
		{
		...
		String message = NetworkProtocol.START_GAME+"&"+shapes.length;
