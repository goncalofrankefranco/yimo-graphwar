...
public class Function
{
	...
	private void calculateGlobalStep(int mode, int index, double stepSize)
	{
		double x = valuesX[index-1];
		double y = valuesY[index-1];
		valuesX[index] = x + stepSize;

		if(mode == Constants.NORMAL_FUNC)
		{
			valuesY[index] = polishFunc.evaluateFunction(valuesX[index], 0, 0);
			valuesDY[index] = 0;
		}
		else if(mode == Constants.FST_ODE)
		{
			double k1 = polishFunc.evaluateFunction(x, y, 0);
			double k2 = polishFunc.evaluateFunction(x + 0.5*stepSize, y + 0.5*stepSize*k1, 0);
			double k3 = polishFunc.evaluateFunction(x + 0.5*stepSize, y + 0.5*stepSize*k2, 0);
			double k4 = polishFunc.evaluateFunction(x + stepSize, y + stepSize*k3, 0);
			valuesY[index] = y + (stepSize/6)*(k1 + 2*k2 + 2*k3 + k4);
			valuesDY[index] = 0;
		}
		else
		{
			double dy = valuesDY[index-1];
			double k11 = dy;
			double k12 = polishFunc.evaluateFunction(x, y, dy);
			double k21 = dy + 0.5*stepSize*k12;
			double k22 = polishFunc.evaluateFunction(x + 0.5*stepSize, y + 0.5*stepSize*k11, k21);
			double k31 = dy + 0.5*stepSize*k22;
			double k32 = polishFunc.evaluateFunction(x + 0.5*stepSize, y + 0.5*stepSize*k21, k31);
			double k41 = dy + stepSize*k32;
			double k42 = polishFunc.evaluateFunction(x + stepSize, y + stepSize*k31, k41);
			valuesY[index] = y + (stepSize/6)*(k11 + 2*k21 + 2*k31 + k41);
			valuesDY[index] = dy + (stepSize/6)*(k12 + 2*k22 + 2*k32 + k42);
		}
	}

	private void recordGlobalHits(Player players[], int numPlayers, int currentTurn, int step, double x, double y)
	{
		double planeX = Constants.PLANE_LENGTH*x/Constants.PLANE_GAME_LENGTH + Constants.PLANE_LENGTH/2;
		double planeY = -Constants.PLANE_LENGTH*y/Constants.PLANE_GAME_LENGTH + Constants.PLANE_HEIGHT/2;

		for(int j=0; j<numPlayers; j++)
		{
			for(int k=0; k<players[j].getNumSoldiers(); k++)
			{
				if(j == currentTurn && k == players[j].getCurrentTurnSoldierIndex())
				{
					continue;
				}
				if(players[j].getSoldiers()[k].isAlive())
				{
					double distX = players[j].getSoldiers()[k].getX() - planeX;
					double distY = players[j].getSoldiers()[k].getY() - planeY;
					if(distX*distX + distY*distY < Constants.SOLDIER_RADIUS*Constants.SOLDIER_RADIUS
							&& playerAlreadyHit(j, k) == false)
					{
						playersHit[numPlayersHit] = j;
						soldiersHit[numPlayersHit] = k;
						soldierHitPosition[numPlayersHit] = step;
						numPlayersHit++;
					}
				}
			}
	...
	public void processGlobalRange(Obstacle obstacle, Player players[], int numPlayers, int currentTurn, int mode)
	{
		playersHit = new int[numPlayers*Constants.MAX_SOLDIERS_PER_PLAYER];
		soldiersHit = new int[numPlayers*Constants.MAX_SOLDIERS_PER_PLAYER];
		soldierHitPosition = new int[numPlayers*Constants.MAX_SOLDIERS_PER_PLAYER];
		...
		valuesY = new double[Constants.FUNC_MAX_STEPS];
		valuesDY = new double[Constants.FUNC_MAX_STEPS];
		valuesX[0] = -Constants.PLANE_GAME_LENGTH/2.0;
		valuesY[0] = mode == Constants.NORMAL_FUNC
				? polishFunc.evaluateFunction(valuesX[0], 0, 0) : 0;
		valuesDY[0] = 0;
		fireAngle = 0;
		offSet = 0;
		numSteps = 1;

		double maxX = Constants.PLANE_GAME_LENGTH/2.0;
		for(int i=1; i<Constants.FUNC_MAX_STEPS; i++)
		{
			double stepSize = Constants.STEP_SIZE;
			calculateGlobalStep(mode, i, stepSize);

			while(stepSize > Constants.FUNC_MIN_X_STEP_DISTANCE
					&& (valuesX[i]-valuesX[i-1])*(valuesX[i]-valuesX[i-1])
					+ (valuesY[i]-valuesY[i-1])*(valuesY[i]-valuesY[i-1]) > Constants.FUNC_MAX_STEP_DISTANCE_SQUARED)
			{
				stepSize = stepSize/2;
				calculateGlobalStep(mode, i, stepSize);
			}

			if(valuesX[i] > maxX || Double.isNaN(valuesY[i]) || Double.isInfinite(valuesY[i]))
			{
				numSteps = i;
				break;
			}

			double planeX = Constants.PLANE_LENGTH*valuesX[i]/Constants.PLANE_GAME_LENGTH + Constants.PLANE_LENGTH/2;
			...
			if(inMap)
			{
				...
				if(obstacle.collidePoint((int)planeX, (int)planeY))
				{
					break;
				}
			}
