...
public class GameData implements Runnable
{
	...
	private boolean customMapEnabled;
	...
	public GameData(Graphwar graphwar)
	{
		...
		previewEnabled = true;
		turnTime = Constants.DEFAULT_TURN_TIME;
	...
	{
		long time = turnTime - (System.currentTimeMillis() - timeTurnStarted);
		
		if(drawingFunction || exploding)
		{
			time = turnTime - (timeStartedDrawingFunction - timeTurnStarted);
		}
	...
	public boolean isGlobalTrajectory()
	{
		return trajectoryMode == Constants.GLOBAL_TRAJECTORY;
	}

	public boolean isCustomMapEnabled()
	{
		return customMapEnabled;
	}

	public MapShape[] getCustomMap()
	{
		return customMap.clone();
	}
	...
	public void setPreviewEnabled(boolean enabled)
	{
		...
		{
			serverConnection.sendMessage(NetworkProtocol.SET_PREVIEW+"&"+(enabled ? 1 : 0));
		}
	}

	public void setTurnTime(int milliseconds)
	{
		if(leader && gameState == Constants.PRE_GAME && serverConnection != null)
		{
			if(milliseconds < Constants.MIN_TURN_TIME)
			{
				milliseconds = Constants.MIN_TURN_TIME;
			}
			if(milliseconds > Constants.MAX_TURN_TIME)
			{
				milliseconds = Constants.MAX_TURN_TIME;
			}
			serverConnection.sendMessage(NetworkProtocol.SET_TURN_TIME+"&"+milliseconds);
		}
	}
	...
	public void setTrajectoryMode(int mode)
	{
		...
		if(leader && gameState == Constants.PRE_GAME && serverConnection != null)
		{
			serverConnection.sendMessage(NetworkProtocol.SET_TRAJECTORY_MODE+"&"+mode);
		}
	}

	public void setCustomMap(MapShape[] shapes)
	{
		if(shapes == null || shapes.length > MapShape.MAX_SHAPES || serverConnection == null
				|| leader == false || gameState != Constants.PRE_GAME)
		{
			return;
		}
		String message = NetworkProtocol.SET_MAP+"&1&"+shapes.length;
		for(int i=0; i<shapes.length; i++)
		{
			if(shapes[i] == null || shapes[i].isWithinMap() == false)
			{
				return;
			}
			String[] fields = shapes[i].encode().split(",");
			for(int j=0; j<fields.length; j++)
			{
				message = message+"&"+fields[j];
			}
		}
		serverConnection.sendMessage(message);
	}
	...
	private void setPreviewMessage(String[] info) throws Exception
	{
		previewEnabled = Integer.parseInt(info[1]) != 0;
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).setRoomPreviewEnabled(previewEnabled);
	}

	private void setTurnTimeMessage(String[] info) throws Exception
	{
		turnTime = Integer.parseInt(info[1]);
		if(turnTime < Constants.MIN_TURN_TIME)
		{
			turnTime = Constants.MIN_TURN_TIME;
		}
		if(turnTime > Constants.MAX_TURN_TIME)
		{
			turnTime = Constants.MAX_TURN_TIME;
		}
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).setRoomTurnTime(turnTime);
	}

	private void setTrajectoryModeMessage(String[] info) throws Exception
	{
		trajectoryMode = Integer.parseInt(info[1]);
		if(trajectoryMode != Constants.GLOBAL_TRAJECTORY)
		{
			trajectoryMode = Constants.SHOOTER_RELATIVE_TRAJECTORY;
		}
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).setRoomTrajectoryMode(trajectoryMode);
	}

	private void setMapMessage(String[] info) throws Exception
	{
		if(info.length < 3)
		{
			return;
		}
		int enabled = Integer.parseInt(info[1]);
		int numShapes = Integer.parseInt(info[2]);
		if(numShapes < 0 || numShapes > MapShape.MAX_SHAPES || info.length != 3 + 5*numShapes)
		{
			return;
		}
		MapShape[] shapes = new MapShape[numShapes];
		for(int s=0; s<numShapes; s++)
		{
			int offset = 3 + 5*s;
			shapes[s] = MapShape.decode(Integer.parseInt(info[offset]), Integer.parseInt(info[offset+1]),
					Integer.parseInt(info[offset+2]), Integer.parseInt(info[offset+3]), Integer.parseInt(info[offset+4]));
		}
		if(enabled == 0 && numShapes != 0)
		{
			return;
		}
		customMapEnabled = enabled != 0;
		customMap = shapes;
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).setCustomMap(customMap, customMapEnabled);
	}
	...
	private void startGameMessage(String[] info) throws Exception
	{
		...
		int numShapes = Integer.parseInt(info[1]);
		MapShape[] shapes = new MapShape[numShapes];
		for(int s=0; s<numShapes; s++)
		{
			int offset = 2 + 5*s;
		...
		obstacle = new Obstacle(numShapes, shapes);
					
		ListIterator<Player> itr = players.listIterator();
    	
		int i = 2 + 5*numShapes;	// Index of first soldier coordinate
    	while(itr.hasNext())
    	{
    		Player player = itr.next();
	...
	public synchronized Function buildPreviewFunction(String functionString)
	{
		...
		try
		{
			previewFunction = calculateFunction(player, functionString);
		}
		catch(MalformedFunction e)
		{
			previewFunction = null;
	...
	private Function calculateFunction(Player player, String functionString) throws MalformedFunction
	{
		Function calculated = new Function(functionString);

		if(isGlobalTrajectory())
		{
			calculated.processGlobalRange(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, gameMode);
			return calculated;
		}
		...
		{			
			switch(gameMode)
			{
				case Constants.NORMAL_FUNC:
					calculated.processFunctionRange(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, false); 	
				break;
				case Constants.FST_ODE:
					calculated.processRK4Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, false);
				break;
				case Constants.SND_ODE:
					calculated.processRK42Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, player.getCurrentTurnSoldier().getAngle(),false);
				break;
			}			
		}
		else
		{
			...
			{
				case Constants.NORMAL_FUNC:
					calculated.processFunctionRange(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, true); 	
				break;
				case Constants.FST_ODE:
					calculated.processRK4Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, true);
				break;
				case Constants.SND_ODE:
					calculated.processRK42Range(obstacle, players.toArray(new Player[0]), players.size(), currentTurn, player.getCurrentTurnSoldier().getAngle(),true);
				break;
			}			
		}
	...
	private void setLeaderMessage(String[] info) throws Exception
	{
		...
		((PreGameScreen)graphwar.getUI().getScreen(Constants.PRE_GAME_SCREEN)).setRoomSettingsEditable(true);
	...
	{		
		...
		try
		{
			...
			switch(type)
			{
				...
				{
					setPreviewMessage(info);
				}break;

				case NetworkProtocol.SET_TURN_TIME:
				{
					setTurnTimeMessage(info);
				}break;

				case NetworkProtocol.SET_TRAJECTORY_MODE:
				{
					setTrajectoryModeMessage(info);
				}break;
