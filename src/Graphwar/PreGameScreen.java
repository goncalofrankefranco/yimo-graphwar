...
public class PreGameScreen extends JPanel implements ActionListener
{
	...
	{
		roomSettingsPanel = new JPanel(new GridBagLayout());
		roomSettingsPanel.setBounds(225, 220, 255, 205);
		roomSettingsPanel.setOpaque(true);
		roomSettingsPanel.setBackground(new Color(22, 31, 46, 235));
		roomSettingsPanel.setBorder(BorderFactory.createCompoundBorder(
				BorderFactory.createLineBorder(new Color(104, 181, 219), 1),
				BorderFactory.createEmptyBorder(6, 8, 6, 8)));

		JLabel title = new JLabel("Room settings");
		...
		roomSettingsPanel.add(title, titleConstraints);

		previewCheckBox = new JCheckBox("Preview trajectory before firing");
		...
		previewConstraints.gridwidth = 2;
		previewConstraints.anchor = GridBagConstraints.WEST;
		roomSettingsPanel.add(previewCheckBox, previewConstraints);

		JLabel timeLabel = new JLabel("Time to play:");
		timeLabel.setForeground(new Color(215, 226, 238));
		GridBagConstraints labelConstraints = new GridBagConstraints();
		labelConstraints.gridx = 0;
		labelConstraints.gridy = 2;
		labelConstraints.anchor = GridBagConstraints.WEST;
		labelConstraints.insets = new Insets(5, 0, 0, 5);
		roomSettingsPanel.add(timeLabel, labelConstraints);

		turnTimeComboBox = new JComboBox<Integer>(new Integer[] { 10, 20, 30, 45, 60, 90, 120, 180 });
		...
		comboConstraints.gridy = 2;
		comboConstraints.anchor = GridBagConstraints.EAST;
		comboConstraints.insets = new Insets(5, 0, 0, 0);
		roomSettingsPanel.add(turnTimeComboBox, comboConstraints);

		JLabel trajectoryLabel = new JLabel("Trajectory:");
		trajectoryLabel.setForeground(new Color(215, 226, 238));
		GridBagConstraints trajectoryLabelConstraints = new GridBagConstraints();
		trajectoryLabelConstraints.gridx = 0;
		trajectoryLabelConstraints.gridy = 3;
		trajectoryLabelConstraints.anchor = GridBagConstraints.WEST;
		trajectoryLabelConstraints.insets = new Insets(5, 0, 0, 5);
		roomSettingsPanel.add(trajectoryLabel, trajectoryLabelConstraints);

		trajectoryModeComboBox = new JComboBox<String>(new String[] {
				"Shooter-relative", "Global graph (both teams)" });
		trajectoryModeComboBox.addActionListener(this);
		GridBagConstraints trajectoryComboConstraints = new GridBagConstraints();
		trajectoryComboConstraints.gridx = 1;
		trajectoryComboConstraints.gridy = 3;
		trajectoryComboConstraints.anchor = GridBagConstraints.EAST;
		trajectoryComboConstraints.insets = new Insets(5, 0, 0, 0);
		roomSettingsPanel.add(trajectoryModeComboBox, trajectoryComboConstraints);
		...
		mapButtonConstraints.gridx = 0;
		mapButtonConstraints.gridy = 4;
		mapButtonConstraints.anchor = GridBagConstraints.WEST;
		mapButtonConstraints.insets = new Insets(5, 0, 0, 5);
		roomSettingsPanel.add(mapEditorButton, mapButtonConstraints);

		mapStatusLabel = new JLabel("Random map");
		mapStatusLabel.setForeground(new Color(160, 181, 200));
		mapStatusLabel.setFont(new Font("Sans", Font.PLAIN, 10));
		GridBagConstraints mapStatusConstraints = new GridBagConstraints();
		mapStatusConstraints.gridx = 1;
		mapStatusConstraints.gridy = 4;
		mapStatusConstraints.anchor = GridBagConstraints.EAST;
		mapStatusConstraints.insets = new Insets(5, 0, 0, 0);
		roomSettingsPanel.add(mapStatusLabel, mapStatusConstraints);

		JLabel hostLabel = new JLabel("Host controls these settings");
		hostLabel.setForeground(new Color(160, 181, 200));
		hostLabel.setFont(new Font("Sans", Font.ITALIC, 10));
		GridBagConstraints hostConstraints = new GridBagConstraints();
		hostConstraints.gridx = 0;
		hostConstraints.gridy = 5;
		hostConstraints.gridwidth = 2;
		hostConstraints.anchor = GridBagConstraints.WEST;
		hostConstraints.insets = new Insets(4, 0, 0, 0);
		roomSettingsPanel.add(hostLabel, hostConstraints);
	...
	public void setCustomMap(MapShape[] shapes, boolean enabled)
	{
		...
		if(enabled)
		{
			mapStatusLabel.setText("Custom map: "+(shapes == null ? 0 : shapes.length)+" shapes");
		}
		else
		{
			mapStatusLabel.setText("Random map");
		}
	}
	...
	public void actionPerformed(ActionEvent arg0)
	{
		...
		{
			if(arg0.getSource()==previewCheckBox || arg0.getSource()==turnTimeComboBox || arg0.getSource()==trajectoryModeComboBox)
			{
				if(updatingRoomSettings == false && graphwar.getGameData().isLeader())
				{
			...
			else if(arg0.getSource()==mapEditorButton)
			{
				if(graphwar.getGameData().isLeader())
				{
			...
			else if(arg0.getSource()==addLocalPlayer)
			{
